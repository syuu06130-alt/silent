...
-- ============================================================
-- CAMERA SMOOTH TRACKING (RenderStepped dt ベース・フレームレート非依存)
-- smooth: 1=ゆっくり(~2秒) / 5.5=普通(~0.3秒) / 10=完全即時ロック
-- ============================================================
-- dt(デルタ秒)を受け取り指数減衰で補間。
-- alpha = 1 - exp(-speed * dt)  →  フレームレートに依存しない
local function SmoothCameraTo(targetPos, smooth, dt)
    ...
    -- smooth 1 → speed=3  /  smooth 5.5 → speed=16.5  /  smooth 9 → speed=27
    local speed = smooth * 3
    local alpha = 1 - math.exp(-speed * (dt or 0.016))
    Camera.CFrame = curCF:Lerp(tgtCF, math.clamp(alpha, 0, 1))
end
...
local function UpdateESPPlayer(player)
    ...
    -- Tracer: 自分のHumanoidRootPart(お腹) → 相手のHumanoidRootPart(お腹)
    local localChar = LocalPlayer.Character
    local localRoot = localChar and localChar:FindFirstChild("HumanoidRootPart")
    local tracerFrom
    if localRoot then
        local spLocal, visLocal = Camera:WorldToViewportPoint(localRoot.Position)
        if visLocal then
            tracerFrom = Vector2.new(spLocal.X, spLocal.Y)
        end
    end
    -- fallback: 自分がオフスクリーンの場合は画面中央下
    if not tracerFrom then
        tracerFrom = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y * 0.85)
    end
    d.Tracer.From    = tracerFrom
    d.Tracer.To      = rootV   -- 相手のHumanoidRootPart(お腹)スクリーン座標
...
-- ============================================================
-- MAIN AIMBOT LOOP  ※ RenderStepped 使用 (Camera操作はRenderStepped必須)
-- GetNearestTarget は Heartbeat で毎フレーム更新
-- ============================================================

-- ターゲット更新は Heartbeat (物理フレーム)
Connections["TargetUpdate"] = RS.Heartbeat:Connect(function()
    GetNearestTarget()
end)

-- カメラ操作は RenderStepped (描画フレーム・dt付き)
Connections["MainAimbot"] = RS.RenderStepped:Connect(function(dt)
    local t = States.CurrentTarget
    if not t then return end
    local pos = t.Position

    -- Silent Aim: Hitbox拡張 + 視点移動が同時起動
    if States.SilentAim then
        SmoothCameraTo(pos, States.CameraSmooth, dt)
    end

    -- Head Lock: 独立スムーズ設定でHeadに向ける
    if States.HeadLock then
        SmoothCameraTo(pos, States.HeadLockSmooth, dt)
    end

    -- Auto Aim: 独立スムーズ設定でなめらかに追従
    if States.AutoAim then
        SmoothCameraTo(pos, States.AutoAimSmooth, dt)
    end
end)
